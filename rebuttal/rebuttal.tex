...
% Let's start point-by-point with Reviewer 1 --------------------------
	...
	\begin{reply}
		Earlier we have submitted our manuscript to EFM: EFM-D-21-00395, considering the reviewer's suggestion we have modified the manuscript and also changed the title of manuscript as suggested by the reviewer and resubmitted to the same journal EFM.

		Publication in Theor. Appl. Fracture Mechanics 2020 (your reference [14]) is based on the auto stepping algorithm for efficient analysis of brittle fracture problems using phase field approach. Though both of these manuscripts main aim is to give a computationally efficient phase-field method to solve brittle fracture problems. The novelty in both the papers are very different in its own way.
	...
	\begin{reply}
		I am aware of the fact that the plagiarism check found 16 $\%$ overlap with my reference [14]. As both the papers aim is to propose a computationally efficient phase-field fracture method and the authors of both the paper are same, so I believe it is permissible. I can definately assure that both the manuscript have their own novelty.
	\end{reply}

	\subsection*{Minor Issues}

	\begin{point}
	 	\textbf{[Point heading]} Please include a nomenclature in alphabetic order
	\end{point}
